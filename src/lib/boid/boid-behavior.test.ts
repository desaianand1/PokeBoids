...
describe('BoidBehavior', () => {
	...
	describe('Creation', () => {
		it('creates prey with default configuration', () => {
			...
			// Then: Should be created as prey variant
			expect(boid.getVariant()).toBe(BoidVariant.PREY);

			// And: Should have valid movement properties
			expect(boid.getMaxSpeed()).toBeGreaterThan(0);
			expect(boid.getMaxForce()).toBeGreaterThan(0);
			expect(boid.getPerceptionRadius()).toBeGreaterThan(0);

			// And: Should have correct prey stats
		...
		it('creates predator with default configuration', () => {
			...
			// Then: Should be created as predator variant
			...
			// And: Should have correct predator stats with attack
			const stats = boid.getStats();
			expect(stats.health).toBe(TEST_BOID_STATS.health);
			expect(stats.stamina).toBe(TEST_BOID_STATS.stamina);
		...
		it('creates boid with custom configuration', () => {
			...
			const boid = new BoidBehavior({ ...deps, config }, 100, 100, BoidVariant.PREY);

			// Then: Should have increased capabilities with custom configuration
			expect(boid.getMaxSpeed()).toBeGreaterThan(TEST_DEFAULTS.boid.maxSpeed);
			expect(boid.getMaxForce()).toBeGreaterThan(TEST_DEFAULTS.boid.maxForce);
			expect(boid.getPerceptionRadius()).toBeGreaterThan(TEST_DEFAULTS.boid.perceptionRadius);
		});
	});

	describe('Vision', () => {
		it('has directional vision', () => {
			// Given: A boid looking right
			const boid = new BoidBehavior(deps, 0, 0, BoidVariant.PREY);
			boid.setBoidVelocity(deps.vectorFactory.create(1, 0));

			// When: Checking targets in different positions
			const frontTarget = new BoidBehavior(deps, 50, 0, BoidVariant.PREY);
			const backTarget = new BoidBehavior(deps, -50, 0, BoidVariant.PREY);

			// Then: Should only see targets in front
			expect(boid.isInFieldOfView(frontTarget)).toBe(true);
			expect(boid.isInFieldOfView(backTarget)).toBe(false);
		});

		it('has limited vision range', () => {
			// Given: A boid looking right
			const boid = new BoidBehavior(deps, 0, 0, BoidVariant.PREY);
			boid.setBoidVelocity(deps.vectorFactory.create(1, 0));

			// When: Checking targets at different distances
			const nearTarget = new BoidBehavior(deps, 50, 0, BoidVariant.PREY);
			const farTarget = new BoidBehavior(deps, 1000, 0, BoidVariant.PREY);

			// Then: Should only see nearby targets
			expect(boid.isInFieldOfView(nearTarget)).toBe(true);
			expect(boid.isInFieldOfView(farTarget)).toBe(false);
		});
	});
	...
	describe('Stamina System', () => {
		...
		it('recovers stamina during rest', () => {
			...
			for (
				let i = 0;
				i < TEST_TIME_CONFIG.defaultTimeout / TEST_TIME_CONFIG.defaultFrameTime;
				i++
			) {
