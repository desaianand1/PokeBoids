...
export class QuadTreePartitioning implements ISpatialPartitioning {
	...
	private options: { capacity?: number; maxDepth?: number };
	...
	constructor(
		...
		const [defaultWidth, defaultHeight] = [3000, 3000];
		this.createQuadTree(defaultWidth, defaultHeight);
		this.setupWorldBoundsListeners();
	}

	private createQuadTree(width: number, height: number): void {
		const boundary = new Rectangle(0, 0, width, height);
		this.quadTree = new QuadTree(boundary, this.options.capacity ?? 4, this.options.maxDepth ?? 8);
	}
	...
	private setupWorldBoundsListeners(): void {
		...
		this.eventBus.on(
			...
			(data) => {
				this.createQuadTree(data.width, data.height);
			},
			this
		);
